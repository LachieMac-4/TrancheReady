...
const {
  PORT = 3000,
  APP_HOST = 'https://trancheready-hbv7.onrender.com', // set in Render env
  STRIPE_SECRET_KEY = '',
...
// Session helpers
// ────────────────────────────────────────────────────────────────────────────
function signSession(payload, mins = 360) {
  return jwt.sign(payload, JWT_SECRET, { expiresIn: `${mins}m` });
}
function readSession(req) {
  const t = req.signedCookies?.tr_session;
  try { return t ? jwt.verify(t, JWT_SECRET) : null; } catch { return null; }
}
function requirePaid(req, res, next) {
  const s = readSession(req);
  if (!s?.paid) return res.redirect('/pricing');
  req.user = s;
  next();
}
...
function analyze(clientsRaw, txnsRaw) {
  ...
  for (const t of txns) {
    const id = t[idk] ?? t.client_id ?? t.clientid ?? t.id;
    if (!id) continue;
    if (!byClient.has(id)) byClient.set(id, []);
    byClient.get(id).push(t);
  }
  ...
  for (const c of clients) {
    ...
    // Structuring: ≥4 cash 9.6–9.9k within 7 days
    const cash = recent
      .filter(t => {
      ...
      .map(t => dt(t.date || t.posted_at || t.txn_date))
      .sort((a, b) => a - b);
  ...
  const counts = {
    total: scores.length,
    high: scores.filter(x => x.band === 'High').length,
    medium: scores.filter(x => x.band === 'Medium').length,
    low: scores.filter(x => x.band === 'Low').length
  };
...
async function buildPack({ clients, txns }) {
  ...
  const files = {
    ...
    'program.html': Buffer.from(`<!doctype html><meta charset="utf-8"><title>Program</title>
      <style>body{font-family:Inter,system-ui,Arial;padding:20px;line-height:1.45}</style>
      <h1>TrancheReady Evidence</h1>
      <p><b>Build:</b> ${buildId}</p>
      <p><b>Counts:</b> H ${counts.high} / M ${counts.medium} / L ${counts.low} (Total ${counts.total})</p>
      <h3>Top Scores (sample)</h3>
      <pre>${JSON.stringify(scores.slice(0,10), null, 2)}</pre>`)
  };
...
app.post('/upload',
  requirePaid,
  multer({ storage: multer.memoryStorage(), limits: { fileSize: 25 * 1024 * 1024 } }).fields([{ name: 'clients' }, { name: 'transactions' }]),
  async (req, res) => {
    try {
      ...
      res.json({
        ok: true,
        counts: pack.counts,
        top: pack.scores.slice(0, 5),
        cases: pack.cases.slice(0, 20),
        verify_url: `${APP_HOST}/verify/${pack.token}`,
        download_url: `${APP_HOST}/download/${pack.token}`
      });
    } catch (e) {
...
app.post('/api/sample-pack', requirePaid, async (_req, res) => {
  try {
    ...
    console.error('sample-pack', e);
    res.status(500).json({ error: 'Failed to build sample pack' });
  }
});

app.get('/verify/:token', (req, res) => {
  const r = verifyStore.get(req.params.token);
  if (!r) return res.status(404).render('error', { title: 'Not found', message: 'Invalid or expired token.' });
  res.render('verify', { manifest: r.manifest, build_id: r.manifest.build_id });
});
app.get('/download/:token', (req, res) => {
  const r = verifyStore.get(req.params.token);
  if (!r) return res.status(404).send('Invalid or expired token');
  res.setHeader('Content-Type', 'application/zip');
  res.setHeader('Content-Disposition', `attachment; filename="${r.zipName}"`);
  res.end(r.zipBuf);
});

// ────────────────────────────────────────────────────────────────────────────
// ZIP helper (ESM-safe, collects into Buffer)
// ────────────────────────────────────────────────────────────────────────────
